...
var regex = /-(\w)/g;
var camelize = cached(
  (str) => str.replace(regex, (_, c) => c ? c.toUpperCase() : "")
);
...
function computeIndexes(slots, children, isTransition2, footerOffset) {
  ...
  const rawIndexes = [...children].map(
    (elt, idx) => idx >= footerIndex ? elmFromNodes.length : elmFromNodes.indexOf(elt)
  );
  return isTransition2 ? rawIndexes.filter((ind) => ind !== -1) : rawIndexes;
}
...
function getComponentAttributes($attrs, componentData) {
  ...
  const { on, props: props2, attrs: componentDataAttrs } = componentData;
  update("on", on);
  update("props", props2);
...
var readonlyProperties = ["Move", ...eventsListened, ...eventsToEmit].map(
  (evt) => "on" + evt
);
...
var draggableComponent = {
  ...
  mounted() {
    ...
    this._sortable = new sortable_esm_default(this.rootContainer, options);
    this.computeIndexes();
  },
  beforeDestroy() {
